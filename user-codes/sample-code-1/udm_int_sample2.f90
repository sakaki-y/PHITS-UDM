...
! Default variables
!-----------------------------------------------------------------------
character(len=99), parameter :: Name = "my_interaction_2" ! This 'Name' is used in input files.
...
integer, save :: kf_initial(num_initial) = (/ 13, -13 /) ! The kf-codes (particle IDs) of the incident particles causing this interaction.
!-----------------------------------------------------------------------
! User variables
!-----------------------------------------------------------------------
! integer i,j
...
if     (udm_kf_incident ==  13) then
  Xsec_per_atom=1e-6*Z
else if(udm_kf_incident == -13) then
  Xsec_per_atom=1e-6*Z
else
  print*,"error"
  stop
