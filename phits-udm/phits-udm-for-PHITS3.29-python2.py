...
SupportVersion=u"3.29"



###########################################################
DIR_ori=u"../src"      # Original directory
DIR_udm=u"../src-udm"  # New directory for phits-udm
###########################################################
...
if os.path.exists(DIR_udm):
    sys.exit(u"@@@ ERROR: 'src-udm' already exists.")



# ======================================================================
# get version
with open(DIR_ori+u"/main.f") as f:
...
if v!=SupportVersion:
    print u"Version error"
    print u"This python script does not support your version, "+v+u"."
    print u"Supported version is "+SupportVersion+u"."
    sys.exit()


# ======================================================================
os.system(u"cp -r {} {}".format(DIR_ori,DIR_udm))
os.system(u"cp udm_Parameter.f {}/.".format(DIR_udm))
...
os.chdir(DIR_udm)
os.system(u"make clean")


# ======================================================================
def check(f,b,a):
    if not os.path.exists(f):
        sys.exit(u"not found: "+f)
    with open(f) as FILE:
        s=FILE.read()
    if s.count(b)!=1:
        print u"@@@ N =",s.count(b)
...
"""
a=u"""\
                    parz( 142 ) = 1000000.d0 ! y.sakaki 2021/9 1000->1000000
                    dnmax( 12 ) = 1000000.d0 ! y.sakaki 2021/9 1000->1000000
c$$$                    ipara = ipara + 1
c$$$                    ipsq(ipara) = -142
                 end if
                 if( kd13 .eq. 0 ) then
                    parz( 143 ) = 1000000.d0 ! y.sakaki 2021/9 1000->1000000
                    dnmax( 13 ) = 1000000.d0 ! y.sakaki 2021/9 1000->1000000
c$$$                    ipara = ipara + 1
c$$$                    ipsq(ipara) = -143
                 end if
                 if( kd14 .eq. 0 ) then
                    parz( 144 ) = 1000000.d0 ! y.sakaki 2021/9 1000->1000000
                    dnmax( 14 ) = 1000000.d0 ! y.sakaki 2021/9 1000->1000000
"""
check(f,b,a)
filename.append(f)
before  .append(b)
...
# sys.exit()
i=0
for f,b,a in izip(filename,before,after):
    i+=1
    with open(f    ) as FILE: s=FILE.read()
    s=s.replace(b,a)
    with open(f,u"w") as FILE: FILE.write(s)
    print i,f
